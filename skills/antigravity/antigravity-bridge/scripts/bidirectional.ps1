...
param(
    ...
    [Parameter()]
    [string]$WebhookUrl = "http://localhost:3101/antigravity",
...
"@ -ErrorAction SilentlyContinue
    ...
    # Envia ao webhook se configurado e se há resposta
    if ($Webhook -and $response) {
        try {
            $body = @{ text = $response } | ConvertTo-Json -Compress
            $bytes = [System.Text.Encoding]::UTF8.GetBytes($body)
            Invoke-WebRequest -Uri $Webhook -Method POST -Body $bytes `
                -ContentType "application/json; charset=utf-8" `
                -ErrorAction Stop | Out-Null
            Write-Host "Resposta enviada para webhook: $Webhook" -ForegroundColor Cyan
        } catch {
            Write-Warning "Falha ao enviar para webhook: $_"
        }
    }
