...
## 5.7 Provider Identity Integrity (Client-Spoofing Defense)

Some model/coding providers (e.g., Kimi Code) require clients to maintain their **real**
identity (User-Agent / client identifier). Spoofing or forging the client identifier is a
**violation** that can get the API key suspended or the membership revoked.

### Detection
- [ ] Tool/config sets a fake or borrowed User-Agent when calling a provider API
- [ ] Skill instructs masking the real client (e.g., "pretend to be Claude Code")
- [ ] Wrapper rewrites the client identifier before sending requests

### Why it matters
- Provider ToS treat client spoofing as abuse: key suspension (loss of paid quota, possible ban).
- Unlike a model fallback (silent, recoverable), a suspension is external and out of our control.
- Spoofing also breaks provider-side abuse monitoring that protects shared infrastructure.

### Rule
- NEVER spoof or forge the User-Agent / client identifier when calling any provider API.
- If a provider needs a specific header, use the one the tool genuinely is: do not impersonate another product.
- Flag any skill, script, or instruction that asks to mask or fake the client identity.
